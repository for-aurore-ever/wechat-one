...
Page({
   ...
   */
  onLoad(options) {
    let growImg = options.growImg ? JSON.parse(options.growImg) : []
    if(growImg){
      let list=[]
      for(var i=0;i<growImg.length;i++){
        if(growImg[i]!==''){
          list.push(Object.values(growImg[i]))
        }
      }
      this.setData({
        growList:list
    ...
    console.log(this.data.growList)
    // this.setData({
    //   growList:Object.values(JSON.parse(options.growImg))
    // })
    console.log(this.data.growList)
