...
Page({
  ...
  goNext(){
    let growthKey=8
    wx.navigateTo({
      url: '/pages/ParentsStudents/myhonour/myhonour?studentId='+this.data.studentId+'&growthId='+this.data.growthId+'&growthKey='+growthKey
    })
  },
  goStart(){
    var that=this
  ...
  chooseImg3: function (e) {
    ...
    wx.chooseImage({
      ...
      success: function (res) {
        // 返回选定照片的本地文件路径列表，tempFilePath可以作为img标签的src属性显示图片
        var tempFilePaths = res.tempFilePaths;
        that.data.imgList.splice(2,1,res.tempFilePaths)
        that.setData({
          //concat将两个数组连接起来,imgs和获取的图片列表
  ...
  onLoad:async function (options) {
    ...
    console.log(this.data.imgObj)
     if(this.data.imgObj.status==1){
       ...
       let imgs3=config.host+'/getFile?name='+this.data.imgObj.img3
